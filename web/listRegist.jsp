...
<div class="right" id="right-id" style="text-align: center;" >
    ...
    <table border="1" cellpadding="5" style="margin: 0 auto;">
        ...
        <form action="MainController" method="POST">
            ...
            <% if ("MN".equals(loginUser.getRole())) {%>
            <td><a href="MainController?action=Accept&status=RG&logId=<%= list.getLogId()%>&role=<%= loginUser.getRole()%>&userId=<%= loginUser.getUserId()%>" target="target"><input style="cursor: pointer" type="button" value="Accept"></a></td>
            <td><a href="MainController?action=No Accept&status=RG&logId=<%= list.getLogId()%>&role=<%= loginUser.getRole()%>&userId=<%= loginUser.getUserId()%>" target="target"><input style="cursor: pointer" type="button" value="No Accept"></a></td>
                    <% }%>
        </form>
        </tr>
